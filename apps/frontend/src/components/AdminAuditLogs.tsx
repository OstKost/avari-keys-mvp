...
export function AdminAuditLogs() {
  ...
  const getCategoryBadge = (category: AuditLogCategory) => {
    switch (category) {
      case 'auth':
        return (
          <span className="bg-amber-950/80 text-amber-300 border border-amber-600/40 text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full inline-flex items-center space-x-1">
      ...
      case 'keys':
        return (
          <span className="bg-emerald-950/80 text-emerald-300 border border-emerald-600/40 text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full inline-flex items-center space-x-1">
      ...
      case 'profile':
        return (
          <span className="bg-cyan-950/80 text-cyan-300 border border-cyan-600/40 text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full inline-flex items-center space-x-1">
      ...
      case 'admin':
        return (
          <span className="bg-purple-950/80 text-purple-300 border border-purple-600/40 text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full inline-flex items-center space-x-1">
      ...
      default:
        return (
          <span className="bg-[#102833] text-[#A8B4B7] border border-[#1C3945] text-[10px] font-mono px-2.5 py-0.5 rounded-full">
            {category}
          </span>
        );
    }
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <button
          ...
          className="flex items-center space-x-2 px-3.5 py-2.5 rounded-xl border border-rose-800/40 bg-rose-950/20 hover:bg-rose-950/40 text-rose-300 font-mono text-xs uppercase tracking-wider transition shadow-sm self-start sm:self-auto"
        >
      ...
      <div className="bg-[#0D222C]/90 border border-[#1C3945] rounded-2xl p-4 mb-5 shadow-lg space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="relative">
            <UserIcon className="w-3.5 h-3.5 text-[#D9B96E] absolute left-3 top-3 pointer-events-none" />
            <select
              ...
              className="w-full bg-[#06141B] border border-[#1C3945] focus:border-[#D9B96E] rounded-xl pl-9 pr-8 py-2 text-xs text-[#F2F0E8] focus:outline-none font-mono transition appearance-none cursor-pointer"
            >
            ...
            <ChevronDown className="w-3.5 h-3.5 text-[#718187] absolute right-3 top-3 pointer-events-none" />
          </div>

          {/* Category Filter */}
          <div className="relative">
            <Filter className="w-3.5 h-3.5 text-[#D9B96E] absolute left-3 top-3 pointer-events-none" />
            <select
          ...
          {/* Date From */}
          <div className="relative">
            <Calendar className="w-3.5 h-3.5 text-[#D9B96E] absolute left-3 top-3 pointer-events-none" />
            <input
              type="date"
              value={fromDate}
              onChange={(e) => {
              ...
              className="w-full bg-[#06141B] border border-[#1C3945] focus:border-[#D9B96E] rounded-xl pl-9 pr-3 py-2 text-xs text-[#F2F0E8] focus:outline-none font-mono transition [color-scheme:dark]"
              title="Период С"
            />
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 pt-1">
          <div className="relative flex-1 w-full">
            <Search className="w-3.5 h-3.5 text-[#718187] absolute left-3 top-2.5" />
            <input
              type="text"
              value={search}
              onChange={(e) => {
              ...
              className="w-full bg-[#06141B] border border-[#1C3945] focus:border-[#D9B96E] rounded-xl pl-9 pr-9 py-2 text-xs text-[#F2F0E8] placeholder-[#718187] focus:outline-none transition shadow-inner font-sans"
            />
            {search && (
              <button
                onClick={() => {
                ...
                className="absolute right-3 top-2.5 text-[#718187] hover:text-[#F2F0E8]"
              >
          ...
          {hasActiveFilters && (
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-2 text-xs font-mono text-[#D9B96E] hover:text-[#F0D48D] bg-[#102833] hover:bg-[#1C3945] border border-[#D9B96E]/30 rounded-xl transition w-full sm:w-auto justify-center"
            >
            ...
            {logs.map((log) => (
              <tr key={log.id} className="hover:bg-[#102833]/50 transition duration-150">
                ...
                <td className="px-4 py-3.5 whitespace-nowrap">
                  <div className="flex items-center space-x-1.5 text-xs font-mono text-[#F2F0E8]">
                    <Clock className="w-3.5 h-3.5 text-[#718187] flex-shrink-0" />
                ...
                <td className="px-4 py-3.5 whitespace-nowrap">
                  <div className="flex items-center space-x-1.5">
                    <span className="font-semibold text-[#F2F0E8]">{log.username}</span>
                    {log.user_id ? (
                      <span className="text-[10px] font-mono text-[#718187]">#{log.user_id}</span>
                    ) : (
                      <span className="text-[10px] font-mono text-[#718187]">(гость)</span>
                    )}
                  </div>
                </td>
                ...
                {/* Action */}
                <td className="px-4 py-3.5 whitespace-nowrap">
                  <div className="font-medium text-[#F2F0E8]">{getActionTitle(log.action)}</div>
                  <div className="text-[10px] font-mono text-[#718187] mt-0.5">{log.action}</div>
                </td>

                {/* Details */}
                <td className="px-4 py-3.5 max-w-md">
                  <span className="text-[#A8B4B7] leading-relaxed break-words font-sans">
                    {log.details || '—'}
                  </span>
                </td>

                {/* IP Address */}
                <td className="px-4 py-3.5 whitespace-nowrap text-xs font-mono text-[#718187]">
                  <div className="flex items-center space-x-1">
                    <Globe className="w-3 h-3 text-[#718187]" />
